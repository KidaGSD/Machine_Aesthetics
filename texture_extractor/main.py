...
def resolve_paths(config, base_dir):
    ...
    # Prioritize results_base_dir if already set (e.g., by run_lamp_pipeline.py)
    if 'results_base_dir' in resolved_config and os.path.isabs(resolved_config['results_base_dir']):
        results_base = resolved_config['results_base_dir']
    else:
        # Otherwise, determine based on dataset type or default
    ...
    if 'audio_timeline_path' in resolved_config and not os.path.isabs(resolved_config['audio_timeline_path']):
        # Don't join with base_dir if it's already absolute (from run_lamp_pipeline)
        if not os.path.isabs(resolved_config['audio_timeline_path']):
    ...
    for key, suffix in [
        ...
        # Use the suffix from the original config if the key isn't already absolute
        output_subdir_name = config.get(key, suffix)
        resolved_config[key] = os.path.join(results_base, output_subdir_name)


    # Resolve specific output filenames within their directories
    cls_conf = resolved_config.get('classification', {})
    # Use the already resolved classification_output_dir
    cls_output_dir = resolved_config['classification_output_dir']
    if 'high_confidence_list_filename' in cls_conf:
        cls_conf['high_confidence_list_path'] = os.path.join(cls_output_dir, cls_conf['high_confidence_list_filename'])
    if 'classification_results_filename' in cls_conf:
         cls_conf['classification_results_path'] = os.path.join(cls_output_dir, cls_conf['classification_results_filename'])
...
def ensure_directories(config):
    ...
    for d in dirs_to_create:
        # Check if path is absolute before trying to create
        if os.path.isabs(d):
            try:
            ...
            print(f"Warning: Skipping directory creation for relative path: {d}")
...
def run_texture_mapping(config):
    ...
    # Determine texture dataset path based on dataset preference
    dataset_type = config.get('dataset_preference', 'huggingface') # Default to huggingface
    if dataset_type == 'huggingface':
        # Use absolute path logic from run_lamp_pipeline for consistency
        texture_dataset_path = os.path.abspath(os.path.join(script_dir, config.get('hugging_data_dir', 'data/hugging_data'), 'color'))
    elif dataset_type == 'dtd':
        texture_dataset_path = config.get('original_textures_dir') # Should be absolute already
    else: # 'all' - For now, default to huggingface for dynamic retrieval
        print("Warning: 'all' dataset type not fully supported for dynamic retrieval yet. Using huggingface.")
        texture_dataset_path = os.path.abspath(os.path.join(script_dir, config.get('hugging_data_dir', 'data/hugging_data'), 'color'))

    emotion_map_path = config.get('emotion_mapping_path') # Used for fallback VAD mapping
    ...
    if not audio_timeline_path or not os.path.exists(audio_timeline_path):
        ...
        return None
    if not texture_dataset_path or not os.path.isdir(texture_dataset_path):
        print(f"Error: Texture dataset path not found or not a directory: {texture_dataset_path}")
        return None
    ...
    try:
        # Create mapper instance with new arguments
        mapper = AudioTextureMapper(
            audio_timeline_path=audio_timeline_path,
            texture_dataset_path=texture_dataset_path,
            emotion_mapping_path=emotion_map_path,
            overall_emotion=config.get('overall_emotion', 'neutral') # Pass overall emotion
        )
...
def run_parametric_design(config, animation_data_path):
    ...
    try:
        ...
        if results:
            # Find the first result that actually generated parameters
            first_valid_result = next((r for r in results if r.get("parameters_path")), None)
            if first_valid_result:
                first_param_path = first_valid_result["parameters_path"]
                script_path = first_param_path.replace(".json", "_rhino.py")
                generator.generate_rhino_python_script(first_param_path, script_path)
            else:
                print("No valid parametric design results found to generate Rhino script.")
...
def main():
    ...
    # Removed --audio argument as it's handled by run_lamp_pipeline.py
    ...
    parser.add_argument("--output_dir", help="Base directory for all outputs (overrides config).")
    # Add VAD support
    ...
    parser.add_argument("--overall_emotion", type=str, default="neutral", help="Overall emotion label for the audio.") # Added argument

    args = parser.parse_args()

    # Load config
    # config_path is now required as it's passed by run_lamp_pipeline.py
    if not args.config or not os.path.exists(args.config):
         print(f"Error: Config file not found or not specified: {args.config}")
         sys.exit(1)
    config_path = args.config
    config = load_config(config_path)

    # Override config with command line args if provided
    # Note: audio_timeline_path is set in the temp config by run_lamp_pipeline.py
    if args.device: config['device'] = args.device
    if args.output_dir: config['results_base_dir'] = args.output_dir # Allow overriding base dir
    if args.vad_csv: config['vad_csv_path'] = args.vad_csv

    # Set classifier type
    config['classifier_type'] = args.classifier

    # Add dataset preference and overall emotion to config
    config['dataset_preference'] = args.dataset
    config['overall_emotion'] = args.overall_emotion # Store overall emotion from args

    # Resolve relative paths in config - Use config's base_dir for resolving relative paths within it
    # The base_dir for resolve_paths should be the directory containing the config file itself
    config_base_dir = os.path.dirname(os.path.abspath(config_path))
    config = resolve_paths(config, config_base_dir) # Pass config's dir as base

    # Ensure output directories exist (using resolved absolute paths)
    ...
    print(f"Overall Emotion: {config['overall_emotion']}") # Print overall emotion
    ...
    if args.skip_blending and args.animation_data:
        ...
        # Pass the blending config section to the function
