...
async function fetchCsv(url) {
  if (!url) {
    console.warn("[TextureLoader] No URL provided to fetchCsv");
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[TextureLoader] HTTP error ${response.status} fetching CSV`);
      throw new Error(`HTTP error ${response.status}`);
    }
    
    const text = await response.text();
    if (!text || text.trim() === '') {
      console.error("[TextureLoader] Empty response from CSV URL");
...
export function useTextureLoader(top2CsvPath, textureClassificationCsvPath) {
  ...
  // Capture THREE reference in component scope to prevent reference errors
  ...
  const createDefaultTexture = useCallback(() => {
    ...
    // Use the THREE reference from the ref
    const texture = new threeRef.current.DataTexture(data, size, size, threeRef.current.RGBAFormat);
    texture.wrapS = threeRef.current.RepeatWrapping;
    texture.wrapT = threeRef.current.RepeatWrapping;
  ...
  const loadSpecificTexture = useCallback(async (textureInfo) => {
    ...
    try {
      ...
      if (sourceDir === 'normal_grey') {
        ...
        console.log(`[TextureLoader] Using normal_grey texture: ${textureFilename} at path: ${textureUrlPath}`);
      } 
        ...
        if (typeMatch) {
          ...
          console.log(`[TextureLoader] Using DTD gray_texture: ${category}/${textureFilename} at path: ${textureUrlPath}`);
        } else {
        ...
        console.warn(`[TextureLoader] Unknown source directory: ${sourceDir} for ${textureFilename}. Using fallback texture.`);
        // Use default texture for unknown sources
        const defaultTexture = createDefaultTexture();
        const textureSet = { 
          displacementMap: defaultTexture, 
          normalMap: defaultTexture, 
      ...
      // Check if the texture path exists before trying to load it
      console.log(`[TextureLoader] Checking if texture exists at: ${textureUrlPath}`);
      try {
        // We'll use a HEAD request to check if the file exists
        const checkResponse = await fetch(textureUrlPath, { method: 'HEAD' });
        if (!checkResponse.ok) {
          console.warn(`[TextureLoader] Texture file not found at: ${textureUrlPath}, using fallback`);
          const defaultTexture = createDefaultTexture();
          const textureSet = { 
            displacementMap: defaultTexture, 
            normalMap: defaultTexture, 
        ...
        console.warn(`[TextureLoader] Error checking texture existence: ${e.message}`);
        // Continue with loading anyway, the error handler will catch failures
      }

      console.log(`[TextureLoader] Loading texture from URL: ${textureUrlPath}`);
      // Use the THREE reference from the ref to avoid scope issues
      const loader = new threeRef.current.TextureLoader();
  ...
  useEffect(() => {
    // Make a more detailed check for paths
    const missingPaths = [];
    if (!top2CsvPath) missingPaths.push("top2CsvPath");
    if (!textureClassificationCsvPath) missingPaths.push("textureClassificationCsvPath");
    
    if (missingPaths.length > 0) {
      console.warn(`[TextureLoader] Missing required CSV paths for texture loading: ${missingPaths.join(', ')}`);
      // Set default textures for missing paths
      const defaultTex = createDefaultTexture();
      setTextureMaps({ 
    ...
    console.log("[TextureLoader] Starting texture loading with paths:");
    console.log("- top2CsvPath:", top2CsvPath);
    console.log("- textureClassificationCsvPath:", textureClassificationCsvPath);
